...
void my_keycallback(my_data& data,  shared_ptr<app_state> app, int key, bool pressing) {
  // Ignore release.
  if (!pressing) return;
...
void my_click_callback(my_data& data,  shared_ptr<app_state> app, int face, const vec2f& uv,
...
void my_draw_glwidgets(my_data& data,  shared_ptr<app_state> app, opengl_window* win) {
...
int run_app(int argc, const char* argv[]) {
  ...
  auto key_callback = [&data]( shared_ptr<app_state> app, int key, bool pressing) {
    my_keycallback(data, app, key, pressing);
  };
  auto click_callback = [&data]( shared_ptr<app_state> a, int f, vec2f uv, int v, float d) {
    my_click_callback(data, a, f, uv, v, d);
  };
  auto draw_glwidgets = [&data]( shared_ptr<app_state> app, opengl_window* win) {
